...
namespace translate_spa.Controllers.ActionFilters
{
	public class UserAccessFilter : ActionFilterAttribute
	{
		...
		{
			// var hasToken = context.HttpContext.Request.Headers.TryGetValue ("X-Authentication-Token", out StringValues tokenHashValues);
			// if (hasToken || tokenHashValues.Count () != 1)
			// {
			//     throw new UnauthorizedAccessException ("Access denied, due to invalid access-token");
			// }
			// var tokenHash = tokenHashValues.First ();
			//var token = new TokenTranslator().Decrypt(tokenHash);
